...
(**)
(*WeierstrassModel:={0,0,0,DegreeFilter[-27*c4,4],DegreeFilter[-54*c6,6]}*)
(**)
...
(**)
(*MultipleTotal[list_]:=E^Total[Log[list]]*)
(**)
(*DegreeFilter[a_,n_]:=If[a==0,0,*)
(*MultipleTotal[Apply[Times,Apply[Power,#]&/@*)
(*({#[[1]],Mod[#[[2]],n]}&/@FactorInteger[a])]]]*)
(**)
...
(**)
(*PointCurveNegAdd[{x1_,y1_},{x2_,y2_}]:=Which[Denominator[Factor[Expand[MinWeierstrassEq/.y->y1+(y2-y1)/(x2-x1)*(x+1)]]/(x-x1)^2]===1,*)
(*{x1,y1},Denominator[Factor[Expand[MinWeierstrassEq/.y->y1+(y2-y1)/(x2-x1)*(x+1)]]/(x-x2)^2]===1,{x2,y2},*)
(*Length[Solve[{y-y1==(y2-y1)/(x2-x1)*(x+1),MinWeierstrassEq==0,x!=x1,x!=x2},{x,y}]]==0,InfinityPoint,True,*)
(*Flatten[{x,y}/.Solve[{MinWeierstrassEq==0,y-y1==(y2-y1)/(x2-x1)*(x-x1),*)
(*x!=x1,x!=x2},{x,y}]]]*)
(**)
(*PointNegativeDouble[{x1_,y1_}]:=Which[Denominator[Factor[Expand[MinWeierstrassEq/.y->y1+((-D[MinWeierstrassEq,x]/D[MinWeierstrassEq,y])/.x->x1/.y->y1)*(x-x1)]]/(x-3)^3]===1,*)
(*{x1,y1},True,Flatten[{x,y}/.Solve[{MinWeierstrassEq==0,y-y1==-((D[MinWeierstrassEq,x]/D[MinWeierstrassEq,y])/.(x->x1)/.(y->y1))*(x-x1),x!=x1},{x,y}]]]*)
(**)
...
(**)
(*TorsionOrder:=Min[FirstPosition[Table[CurveMulNum[{0,0},n]==InfinityPoint,{n,1,16}],True]]*)
(**)
...
(**)
(*(*Division polynomials. Finished.*)*)
(**)
(*DivisionPolynomial[0]:=0*)
(**)
(*DivisionPolynomial[1]:=1*)
(**)
(*DivisionPolynomial[2]:=2y*)
(**)
(*DivisionPolynomial[3]:=Block[{A=DegreeFilter[-27c4,4],B=DegreeFilter[-54c6,6]},3x^4+6A x^2+12B x -A^2]*)
(**)
(*DivisionPolynomial[4]:=Block[{A=DegreeFilter[-27c4,4],B=DegreeFilter[-54c6,6]},4y(x^6+5A x^4+20B x^3-5A^2x^2-4A B x - 8B^2-A^3)]*)
(**)
(*DivisionPolynomial[n_]:=If[EvenQ[n],Expand[(DivisionPolynomial[n/2]/(2y))*(DivisionPolynomial[n/2+2]*DivisionPolynomial[n/2-1]^2-DivisionPolynomial[n/2-2]*DivisionPolynomial[n/2+1]^2)],*)
(*Expand[DivisionPolynomial[(n-1)/2+2]*DivisionPolynomial[(n-1)/2]^3-DivisionPolynomial[(n-1)/2-1]*DivisionPolynomial[(n-1)/2+1]^3]]*)
(**)
...
(**)
(*(*Trying to calculate L-function via Dokchitser's and Gross-Zagier evaluations. WIP*)*)
(**)
(*A:=Sqrt[Conductor]/Pi*)
(**)
...
(**)
(*LFuncGrossZagierSum[z_]:=Sum[Terms[n]*E^(2*Pi*I*n*z),{n,1,500}]*)
(**)
(*LFuncGrossZagierSum[s_]:=NIntegrate[LFuncGrossZagierSum[I*y/Sqrt[Conductor]]*y^(s-1),{y,0,1000}]*)
(**)
...
(*y^2+newa1[TateChange2[p]]*x*y+newa3[TateChange2[p]]-*)
(*(x^3+Coefficient[ShiftTripleRoot,T,2]*p*x^2+Coefficient[ShiftTripleRoot,T,1]*p^2*x+Coefficient[ShiftTripleRoot,T,0]*p^3)]*)
(**)
